...
function universityDraftCoverage(data) {
  ...
  const groups = [
    /tổng quan|giới thiệu|thông tin cơ bản/,
    /địa chỉ|liên hệ|website/,
    /ngành|khoa|chuyên ngành|đào tạo/,
    /tuyển sinh|điều kiện|hồ sơ/,
    /học phí|chi phí/,
    /học bổng/,
    /ký túc|nhà ở/,
    /thông báo|tin mới|cập nhật/,
    /tài liệu|nguồn chính thức/,
  ];
  const covered = groups.filter((pattern) => headings.some((heading) => pattern.test(heading))).length;
  return text.length >= 2500 && headings.length >= 8 && covered >= 8 && isVietnameseDraft(data);
}
...
1. Nội dung chính thường dài khoảng 1.800–4.500 từ khi nguồn đủ dữ liệu và có thể dài hơn nếu cần để giữ bảng, ngành, điều kiện, lịch, học phí, thông báo và tệp; không dùng số từ làm lý do cắt dữ kiện.
...
${String(input.sourceText || '')}`;
  try {
    ...
    if (!universityDraftCoverage(data)) throw new Error('Hồ sơ trường chưa đạt độ phủ nội dung bắt buộc hoặc vẫn còn ngoại ngữ chưa dịch.');
...
module.exports = { generateEditorialDraft, generateUniversityProfileDraft, resolveOfficialUniversityWebsite, discoverOfficialSourceItems, researchOfficialUrlViaSearch, universityDraftCoverage, fallbackDraft, suggestResearchKeywords, configuredEditorialModels, isRetryableEditorialError, parseOpenAiWebSearchResponse, officialSearchItemsFromJson, searchProviderError, criticalSourceFacts, sourceFidelityReport, sourceClassificationSummary, sourceBlockCoverageReport, FALLBACK_KEYWORDS };
